...
OUT_DIR="CIRPIN/AFDB_ClustR/embs_progres_${SLURM_ARRAY_TASK_ID}"
mkdir -p "$OUT_DIR"

TEMP_PDB_DIR="CIRPIN/AFDB_ClustR/temp_pdb_progres_${SLURM_ARRAY_TASK_ID}"
mkdir -p "$TEMP_PDB_DIR"


# Run script
python CIRPIN/AFDB_ClustR/fc_embed_afdb_clustr_reps_Progres.py --input "$CHUNK_FILE" --output "$OUT_DIR" --temp_dir "$TEMP_PDB_DIR"